...
// Hover AND focus-visible get the same treatment: the old inline
// onMouseEnter/onMouseLeave recolouring left a keyboard user with no way to
// tell which of the two icon buttons they had landed on.
const ICON_BTN =
...
export default function CostCentresPage() {
  ...
  // Handed down to FormDialog so Ctrl+S can submit whichever form is open.
  // Registering the chord here rather than inside the dialog keeps ONE hint
  // list for the screen — registerHints replaces, so a second registration
  // would blank the page's chips when the dialog closed.
  const categoryFormRef = useRef<HTMLFormElement>(null)
  const centreFormRef = useRef<HTMLFormElement>(null)
  ...
  function newCategory() { setEditingCategory(null); setShowCategoryForm(true) }
  function newCentre() { setEditingCentre(null); setShowCentreForm(true) }
  function editCategory(c: CostCategory) { setEditingCategory(c); setShowCategoryForm(true) }
  function editCentre(c: CostCentre) { setEditingCentre(c); setShowCentreForm(true) }

  // ─── Keyboard ──────────────────────────────────────────────────────────────
  // Two registers, each with a roving tabindex: reaching the 20th centre's Edit
  // button used to cost ~40 Tab presses, because every row carried two stops
  // and the rows themselves could not be focused at all.
  const categoryNav = useListKeyboardNav({
    count: categories.length,
    onActivate: (i) => editCategory(categories[i]),
  })
  const centreNav = useListKeyboardNav({
    count: centres.length,
    onActivate: (i) => editCentre(centres[i]),
  })

  const formOpen = showCategoryForm || showCentreForm

  usePageKeyboard({
    actions: [
      // Alt+N is the page's primary record. The secondary create takes a free
      // mnemonic (G for the group a centre sits under) rather than Alt+C: that
      // chord means "clear filters" on a list and "create a ledger" inside a
      // voucher, and a third meaning here would be a third thing to unlearn
      // even though this screen happens to have no filters to clear.
      { chord: 'Alt+N', label: 'New cost centre', run: newCentre, when: categories.length > 0 },
      { chord: 'Alt+G', label: 'New category', run: newCategory },
      { chord: 'Alt+R', label: 'Refresh', run: load },
      {
        chord: 'Ctrl+S',
        label: 'Save',
        when: formOpen,
        run: () => (showCategoryForm ? categoryFormRef : centreFormRef).current?.requestSubmit(),
      },
    ],
    // F3 aims at the page's subject; the categories table above it is one Tab
    // away and arrow-navigable in its own right.
    onFocusList: () => (centres.length ? centreNav.focusList() : categoryNav.focusList()),
    onBack: () => navigate(-1),
    // While a form dialog owns the screen, Escape belongs to it.
    backActive: !formOpen,
  })
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="secondary" onClick={newCategory}>
            <Plus size={14} /> Category
          </Button>
          <Button onClick={newCentre}
        ...
        <>
          ...
          <Card className="p-0 overflow-hidden">
            ...
            {categories.length === 0 ? (
              ...
              // Focusable rail: on a narrow window the right-hand columns are
              // only reachable by scrolling this box, and without a tab stop the
              // keyboard could never scroll it.
              <div className="table-scroll" tabIndex={0} role="region" aria-label="Cost categories, scrollable">
                <table className="w-full text-sm min-w-[560px]">
                  ...
                  <tbody {...categoryNav.containerProps}>
                    {categories.map((c, i) => {
                      const rowKb = categoryNav.rowProps(i)
                      return (
                      <tr
                        key={c.id}
                        className="border-b last:border-0"
                        style={{ borderColor: 'var(--line)' }}
                        aria-label={`Category ${c.name} — Enter to edit`}
                        {...rowKb}
                        // Keyboard only: the row is arrow-navigable and Enter
                        // on it edits, while a click stays inert as it always
                        // was — the Edit button is the pointer's target. Enter
                        // on one of those icon buttons belongs to the button.
                      ...
                      >
                        ...
                        <td className="px-2 py-2.5">
                          {/* The recolour was bound to onMouseEnter/Leave, which
                              no keyboard event fires — as CSS, :focus-visible
                              gets the same treatment :hover does. */}
          ...
          <Card className="p-0 overflow-hidden">
            ...
            {centres.length === 0 ? (
              ...
              <div className="table-scroll" tabIndex={0} role="region" aria-label="Cost centres, scrollable">
                <table className="w-full text-sm min-w-[680px]">
                  ...
                  <tbody {...centreNav.containerProps}>
                    {centres.map((c, i) => {
                      const rowKb = centreNav.rowProps(i)
                      return (
                      <tr
                        key={c.id}
                        className="border-b last:border-0"
                        style={{ borderColor: 'var(--line)' }}
                        aria-label={`Cost centre ${c.name} — Enter to edit`}
                        {...rowKb}
                        onKeyDown={(e) => { if (e.target === e.currentTarget) rowKb.onKeyDown(e) }}
                      >
                        <td className="px-4 py-2.5 font-medium" style={{ color: 'var(--ink)' }}>{c.name}</td>
                        ...
                        <td className="px-2 py-2.5">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              type="button"
                              aria-label={`Edit cost centre ${c.name}`}
                              className={ICON_BTN}
                              onClick={() => editCentre(c)}
                            >
                              <Pencil size={13} />
                            </button>
                            <button
                              type="button"
                              aria-label={`Delete cost centre ${c.name}`}
                              className={ICON_BTN_DANGER}
...
function CategoryForm({ open, onOpenChange, editing, onSaved, formRef }: {
  open: boolean
...
function CentreForm({ open, onOpenChange, editing, categories, centres, onSaved, formRef }: {
  open: boolean
  onOpenChange: (open: boolean) => void
...
function FormDialog({ open, onOpenChange, title, onSubmit, saving, children, formRef }: {
  open: boolean
  onOpenChange: (open: boolean) => void
  title: string
