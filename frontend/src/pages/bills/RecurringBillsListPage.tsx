      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="relative flex-1 max-w-md">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search profile name or vendor…" className="pl-9 py-1.5" />
        </div>
      </div>
          ...
          <Tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center py-12 text-slate-400 text-sm">
                {filter === 'all' ? 'No recurring profiles yet. Create one to schedule monthly rent, utilities, etc.' : 'No profiles match your filter'}
              </td></tr>
            ) : profiles.map((p) => {
              const isDue = p.status === 'active' && p.next_run_date <= today
              return (
                <Tr key={p.id} className="cursor-pointer hover:bg-slate-50" onClick={() => navigate(`/bills/recurring/${p.id}`)}>
                  <Td>
                    ...
                    {p.auto_approve && (
                      <span className="ml-2 text-[10px] uppercase tracking-wide text-emerald-700">Auto-approve</span>
                    )}
                  </Td>
                  <Td className="text-sm text-slate-700">{p.vendor_name}</Td>
                  ...
                  <Td className="text-right font-mono px-3">{formatCurrency(p.total_amount)}</Td>
